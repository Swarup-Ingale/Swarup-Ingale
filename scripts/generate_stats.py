...
Env:
  ...
  GH_LOGIN      user to summarise (default: Swarup-Ingale)
...
# ACCENT colour: subtle cyber-green for the sparkline endpoint
ACCENT = "#00cc5c"
ACCENT_DARK = "#39d353"
...
def style(extra="", font=None):
    ...
    return (f"<style>{font or font_text()}"
            ...
            f".a-f{{fill:{ACCENT}}}.a-s{{stroke:{ACCENT}}}{extra}"
            f"@media(prefers-color-scheme:dark){{{block(DARK)}"
            f".w{{fill:{DARK['data']};opacity:.16}}"
            f".a-f{{fill:{ACCENT_DARK}}}.a-s{{stroke:{ACCENT_DARK}}}}}</style>")
...
def draw_stats(s):
    ...
    p.append("</g>")
    p.append(cursor)
    # Accent-coloured endpoint circle instead of default grey
    ex, ey = pts[-1]
    p.append(f'<circle cx="{ex - 2:.1f}" cy="{ey:.1f}" r="4.5" class="a-f r" '
...
def draw_heading(word):
    ...
    # Use accent colour for the heading rule line
    p.append(f'<line x1="{text_end:.0f}" y1="12.5" x2="{WIDTH}" y2="12.5" '
             f'class="a-s" stroke-width="1" opacity="0.4"/>')
    p.append("</svg>")
    return "".join(p)
...
def main():
    ...
    login = os.environ.get("GH_LOGIN", "Swarup-Ingale")
    ...
    for word in ("recon", "arsenal", "operations", "trophies", "telemetry", "comms"):
